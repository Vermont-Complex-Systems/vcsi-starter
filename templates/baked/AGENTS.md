...
A scrollytelling website built on [@the-vcsi/scrolly-kit](https://www.npmjs.com/package/@the-vcsi/scrolly-kit) (SvelteKit 2 + Svelte 5 runes). This is the **baked** template: static, prerendered at build time (`adapter-static`).
...
Every page is prerendered. Guard browser-only code (`window`/`document`/WebGL, e.g. DuckDB) with `{#if browser}` (`import { browser } from '$app/environment'`). Do **not** set `export const ssr = false` globally — it stops remote functions from being crawled and produces 404s at runtime.
