...
class DuckQuery {
  ...
  constructor(table: string, filters: FilterFn[] = [], orderBy: string[] = []) {
    this._table = table;
  ...
  arrange(...cols: string[]) {
    return new DuckQuery(this._table, this._filters, cols);
  }
  ...
  rows<T = Record<string, unknown>>() {
    return duck<T>(() => `SELECT * FROM ${this._table} ${this._where()} ${this._order()}`);
  }

  /** First N rows (like R's tibble print). Default 6. */
  head<T = Record<string, unknown>>(n: number = 6) {
    return duck<T>(() => `SELECT * FROM ${this._table} ${this._where()} ${this._order()} LIMIT ${n}`);
  }
  ...
  _add(filter: FilterFn): DuckQuery {
    return new DuckQuery(this._table, [...this._filters, filter], this._orderBy);
  }
